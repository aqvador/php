<div class="container">
<h2 class="your_comments">Вы можете оставить тут комментарий =)</h2>
    <div class="comments">
    <form action="">
        <h4>Ваше имя:</h4> <input type="text" id="mess_name" name="name" maxlength="50">
    ...
    <?php 
    require_once 'gallery/config/config.php';
    require_once 'gallery/function/function.php';
    
    $uri = $_SERVER['REQUEST_URI'];
    $result = substr($uri, strpos($uri, '=') + 1, strlen($uri));
    if ($result == '') {
        $result = 'ttp://lesson_6.geekbrains.club/';
    }
...
?>
    ...
    function comment() {
        ...
        $.ajax({
    method:"GET",
    url:"/calc.php?comment_form=" + text + "&name_form=" + name
    })
